...
for j in range(len(radius_values)-1):
    i = len(radius_values)-2 - j  # want to start at the bottom
    r = radius_values[i]
    h = r - radius_values[i+1]
    nz = nz_layers[i]
    dz = h / nz
...
if args.refined_surface:
    surface_mesh = Mesh(f"./weerdesteijn_box_refined_surface_{round(args.dx)}km.msh", name="surface_mesh")
else:
    nx = round(L / (args.dx*1e3))
    surface_mesh = SquareMesh(nx, nx, L)
...
def initialise_background_field(field, background_values):
    for i in range(0, len(background_values)):
        field.interpolate(conditional(vc(X) >= radius_values[i+1] - radius_values[0],
                          conditional(vc(X) <= radius_values[i] - radius_values[0],
...
approximation = CompressibleInternalVariableApproximation(bulk_modulus, density, shear_modulus, viscosity, g=g)
...
checkpoint_filename = f"{args.output_path}{name}-refinedsurface{args.refined_surface}-dx{args.dx}km-nz{nz}-dt{dt_years}years-bulk{args.bulk_modulus}-chk.h5"

displacement_filename = f"{args.output_path}displacement-{name}-refinedsurface{args.refined_surface}-dx{args.dx}km-nz{nz}-dt{dt_years}years-bulk{args.bulk_modulus}.dat"
