...
initialise_background_field(exponent, exponent_values)
...
approximation = CompressibleInternalVariableApproximation(bulk_modulus=bulk_modulus, density=density, shear_modulus=[shear_modulus], viscosity=[viscosity], Vi=Vi, bulk_shear_ratio=args.bulk_shear_ratio, compressible_buoyancy=compressible_buoyancy, compressible_adv_hyd_pre=compressible_adv_hyd_pre, power_law=args.power_law, exponent=exponent, transition_stress=transition_stress)

stress.interpolate(approximation.deviatoric_stress(u, m_list))
...
if OUTPUT:
    ...
    output_file.write(u, m, stress, dev_stress_2, power_factor)
...
checkpoint_filename = f"{args.output_path}{name}-refinedsurface{args.refined_surface}-dx{args.dx}km-nz{nz}-dt{dt_years}years-bulktoshear{args.bulk_shear_ratio}-compbuoy{compressible_buoyancy}-powerlaw{args.power_law}-nondim-chk.h5"

displacement_filename = f"{args.output_path}displacement-{name}-refinedsurface{args.refined_surface}-dx{args.dx}km-nz{nz}-dt{dt_years}years-bulk{args.bulk_shear_ratio}-compbuoy{compressible_buoyancy}-powerlaw{args.power_law}-nondim.dat"
