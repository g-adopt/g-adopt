...
name = f"weerdesteijn-3d-internalvariable-burgers-{args.optional_name}"
...
Z = MixedFunctionSpace([V, S, S])  # Mixed function space.
...
u, m1, m2 = split(z)  # Returns symbolic UFL expression for u and m
# Next rename for output:
z.subfunctions[0].rename("Displacement")
z.subfunctions[1].rename("Internal variable 1")
z.subfunctions[2].rename("Internal variable 2")
# -
...
# Set each component of shear mod and viscosity to half to get same behaviour 
# as original Maxwell model with 1 internal variable... hopefully!
...
viscosity_1 = Function(DG0, name="viscosity")
initialise_background_field(viscosity_1, viscosity_values_1_tilde)

viscosity_2 = Function(DG0, name="viscosity")
...
approximation = CompressibleInternalVariableApproximation(bulk_modulus=bulk_modulus, density=density, shear_modulus=[shear_modulus_1, shear_modulus_2], viscosity=[viscosity_1, viscosity_2], Vi=Vi, bulk_shear_ratio=args.bulk_shear_ratio, compressible_buoyancy=compressible_buoyancy)
...
#                        "fieldsplit_0_pc_python_type": "gadopt.SPDAssembledPC",
                        ...
                        "fieldsplit_2_assembled_pc_type": "sor",
