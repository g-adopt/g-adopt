...
def summarize_coupled(rows):
    ...
    out = ["=== Coupled solve (weak-scaling axis, L <= 6) ===", ""]
    out.append("fieldsplit_0 iterations per invocation (flat across level = AMG scales):")
    ...
    out.append("")
    out.append("mean timed wall time (s):")
    out.append(_grid(rows, "level", "lmax",
                     lambda r: f"{sum(r['wall_times']) / len(r['wall_times']):.1f}"))
    return "\n".join(out)
...
def main(results_dir):
    ...
    print(summarize_coupled(coupled))
    print()
    print(summarize_capacitance(capacitance))
    print()
    excitation = check_excitation(coupled)
    print(excitation)
    print()
