...
MARTINEC_DRIVER = (Path(__file__).resolve().parents[2] / "tests"
                   / "gia_selfgrav_benchmarks" / "martinec.py")
...
def load_martinec_driver():
    ...
    The driver is a benchmark script and not part of the package, so it is
    loaded by path, the way `load_will_sea_level` loads Will Scott's solver.
    Its module body puts its own directory on `sys.path` and imports the
    shared module `selfgrav_common` from there. It also imports `giamip`,
    which is in the `demos` extra of G-ADOPT.
    """
    if not MARTINEC_DRIVER.exists():
        pytest.skip(f"The Martinec driver is not at {MARTINEC_DRIVER}.")
    spec = importlib.util.spec_from_file_location("martinec",
                                                  MARTINEC_DRIVER)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module
