...
class TestPolygonSourceRegression:
    def test_prepare_matches_reference(self, polygon_source):
        ...
        for age in TEST_AGES:
            ...
            ref = reference[age]
            for key, expected in ref.items():
                if key in ("thickness_sum", "thickness_nonzero"):
                    # Deterministic invariants: every in-polygon seed survives
                    # the rotation and carries its original thickness, so the
                    # seed count (thickness_nonzero) and the total (sum over the
                    # kept seeds; the fresh background contributes 0) do not
                    # depend on the background grid and must match tightly.
                    np.testing.assert_allclose(
                        reduced[key], expected, rtol=1e-6,
                        err_msg=f"age {age}: {key}",
                    )
                else:
                    # n_points and thickness_mean depend on how many fresh
                    # background points survive collision removal, which shifts
                    # by a handful of points with the rotated-seed positions
                    # (and across platforms). Allow ~1% rather than pinning it.
                    np.testing.assert_allclose(
                        reduced[key], expected, rtol=0.01,
                        err_msg=f"age {age}: {key}",
                    )
...
class TestPolygonSourceTopological:
    """Acceptance checks for the topological PolygonSource: the rotator drops
    no in-polygon seeds, and build_indicator_source keeps the background dense
    (no coverage holes) at every age. These pin the two failures the rigid
    full-grid path produced — dropped out-of-circuit seeds and a tens-of-
    degrees coverage hole that let the kNN smear a distant craton."""

    def test_seeds_survive_rotation(self, polygon_source):
        # The topological rotator advects every in-polygon seed; the old rigid
        # path silently dropped out-of-circuit ones. Rotating the seed cloud to
        # a non-trivial age must return exactly as many points as it was given.
        polygon_source._ensure_loaded()
        if not polygon_source.is_root:
            pytest.skip("rank-0-only internal state")
        seeds = polygon_source._region_present
        assert seeds.n_points > 0
        for age in (50.0, 100.0):
            rotated = polygon_source._rotator.rotate(
                seeds, from_age=0.0, to_age=age
            )
            assert rotated.n_points == seeds.n_points, (
                f"age {age}: rotation dropped "
                f"{seeds.n_points - rotated.n_points} seeds"
            )

    def test_background_coverage_has_no_holes(self, polygon_source):
        # The fresh background regenerated at the target age must stay dense:
        # the largest nearest-neighbour gap in the combined cloud should be of
        # order the nominal grid spacing, not the tens-of-degrees hole a
        # back-rotated fixed grid opens. Bound it at a few grid-spacings.
        nominal_deg = np.degrees(np.sqrt(4.0 * np.pi / POLYGON_N_POINTS))
        for age in (50.0, 100.0):
            d = polygon_source.prepare(float(age))
            xyz = np.asarray(d["xyz"])
            unit = xyz / np.linalg.norm(xyz, axis=1, keepdims=True)
            tree = cKDTree(unit)
            # k=2: nearest neighbour is the point itself (distance 0); the
            # second is the true nearest neighbour, i.e. the local coverage gap.
            dist, _ = tree.query(unit, k=2)
            chord = dist[:, 1]
            gap_deg = np.degrees(2.0 * np.arcsin(np.clip(chord / 2.0, 0.0, 1.0)))
            assert gap_deg.max() < 3.0 * nominal_deg, (
                f"age {age}: max coverage gap {gap_deg.max():.2f} deg exceeds "
                f"{3.0 * nominal_deg:.2f} deg (3x nominal spacing)"
            )
