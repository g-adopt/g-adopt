...
def test_parallel_spherical_layer_areas():
    (
        ...
        layer_sums,
        pointwise_errors,
        linear_pointwise_errors,
        linear_sum_errors,
        comm_sizes,
    ) = np.loadtxt(Path(__file__).with_name("layer_sums.dat"), unpack=True)
    ...
    np.testing.assert_allclose(linear_pointwise_errors, 0.0, atol=1.0e-12)
    assert np.all(linear_sum_errors < 1.0e-9)
