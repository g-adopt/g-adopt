...
def generate_mesh(mesh_path: str):
    gmsh.initialize()
    gmsh.model.add("mesh")

    point_1 = gmsh.model.geo.addPoint(0.0, 0.0, 0.0, mesh_res["vertical"])
    point_2 = gmsh.model.geo.addPoint(0.0, domain_dims[1], 0.0, mesh_res["vertical"])

    line_1 = gmsh.model.geo.addLine(point_1, point_2)

    for i, (layer_width, layer_resolution) in enumerate(
        zip(layer_widths, layer_resolutions)
    ):
        gmsh.model.geo.extrude(
            ...
            0.0,
            0.0,
            numElements=[int(layer_width / layer_resolution)],
            recombine=True,
        )
...
def diagnostics(simu_time: float, geo_diag, diag_vars: dict, output_path: str):
    ...
    diag_fields["entrainment"].append(
        material_entrainment(
            ...
            material_size=diag_params["material_size"],
...
def plot_diagnostics(output_path: str):
    if MPI.COMM_WORLD.rank == 0:
...
def symmetric_cubic(
    ...
    cubic = amplitude * (support / 2.0 - abs(t - centre)) ** 3 / (support / 2.0) ** 3
...
mesh_gen = "gmsh"
mesh_res = {"horizontal": 1e3, "vertical": 1e2}
fine_layer = {"width": 2.4e4, "resolution": 1e2}
layer_widths = [
    (domain_dims[0] - fine_layer["width"]) / 2.0,
    fine_layer["width"],
    (domain_dims[0] - fine_layer["width"]) / 2.0,
]
layer_resolutions = [
    mesh_res["horizontal"],
    fine_layer["resolution"],
    mesh_res["horizontal"],
]

# Parameters to initialise level set
callable_args = (
    curve_parameter := np.linspace(0.0, domain_dims[0], 100),
    perturbation_centre := domain_dims[0] / 2.0,
    perturbation_support := 6e3,
    interface_deflection := 1.0,
    interface_coord_y := 2.5e3,
)
...
stokes_bcs = {1: {"ux": 0.0}, 2: {"ux": 0.0}, 3: {"u": 0.0}, 4: {"uy": 0.0}}
...
diag_params = {
    "material_size": domain_dims[0] * interface_coord_y,
    "entrainment_height": interface_coord_y,
}
