...
if simulation.checkpoint_restart:  # Restore mesh and key functions
    ...
    if args.benchmark == "trim_2023":
        epsilon = Constant(1 / 2 / simulation.k)
    ...
    func_space_ls = FunctionSpace(mesh, "DQ", 2)
    level_set = [
...
func_space_output = FunctionSpace(mesh, "CG", 2)
output_fields = []
...
if simulation.dimensional:
    ...
    density = Function(func_space_output, name="Density").interpolate(rho_material)
    ...
    approximation_parameters["Ra_c"] = 1.0
else:
    Ra_c = material_field(
        level_set,
    ...
    compo_rayleigh = Function(func_space_output, name="Ra_c").interpolate(Ra_c)
...
viscosity = Function(func_space_output, name="Viscosity").interpolate(mu)
output_fields.append(viscosity)
approximation_parameters["mu"] = mu

if args.benchmark == "trim_2023":
    H = Function(temperature.function_space(), name="Internal heating rate")
...
Ra = getattr(simulation, "Ra", 1.0)

approximation = BoussinesqApproximation(Ra, **approximation_parameters)
...
stokes_nullspace = create_stokes_nullspace(stokes_function.function_space())
...
level_set_solver = [
    LevelSetSolver(ls, velocity, timestep, eSSPRKs10p3, epsilon) for ls in level_set
]
if args.benchmark == "trim_2023":
...
geo_diag = GeodynamicalDiagnostics(stokes_function, temperature, bottom_id=3, top_id=4)
...
while True:
    ...
    if has_end_time:
        ...
        exit_loop = norm(velocity - velocity_old) < simulation.steady_state_threshold
        velocity_old = stokes_function.subfunctions[0].copy(deepcopy=True)
