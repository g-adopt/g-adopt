...
def model(level, nn, do_write=False):
    ...
    rmin, rmax = 1.22, 2.22
    rp = Constant((rmin + rmax) / 2.0)  # height of forcing: delta(r-rp)
    nn = Constant(nn)
    ...
    Nvec = VectorFunctionSpace(mesh, "CG", 1)  # pressure function space (scalar)
    ...
    solution_upper = assess.CylindricalStokesSolutionDeltaZeroSlip(
        float(nn), +1, nu=float(mu)
    )
    solution_lower = assess.CylindricalStokesSolutionDeltaZeroSlip(
        float(nn), -1, nu=float(mu)
    )
    ...
    sigmaxy = Function(Nvec).interpolate(X)
    sigma_anal_upper = Function(N, name="AnalyticalNormalStressUpper")
    sigma_anal_lower = Function(N, name="AnalyticalNormalStressLower")
    ...
    if do_write:
        ...
        sigma_file = VTKFile("zs_normalstress_{}.pvd".format(level))
