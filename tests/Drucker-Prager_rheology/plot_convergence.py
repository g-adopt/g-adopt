...
def spiegelman(U0, mu1, nx, ny, picard_iterations, stabilisation=False):
    ...
    residuals += [
        float(line[23:]) for line in open(os.path.join(output_dir, "newton.txt"))
    ]
    return np.array(residuals)


for ui, mui, mu_latex in zip(
    [2.5e-3, 5e-3, 12.5e-3],
    [1e23, 1e24, 5e24],
    ["10^{23}", "10^{24}", r"5\times 10^{24}"],
):
    plt.figure()
    for picard_iterations, color in zip(
        [50, 0, 5, 15, 25], ["tab:purple", "k", "b", "tab:orange", "r"]
    ):
        for stab in [False, True]:
            if stab and picard_iterations == 50:
                break
            residuals = spiegelman(
                ui, mui / mu0, nx, ny, picard_iterations, stabilisation=stab
            )
