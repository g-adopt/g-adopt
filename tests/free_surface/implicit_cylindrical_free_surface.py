...
class CylindricalImplicitFreeSurfaceModel(ImplicitFreeSurfaceModel):
    ...
    def initialise_wavenumber(self):
        # number of waves
        self.number_of_lam = 4 * round(2 * pi * self.rmax / (self.rmax - self.rmin))
    ...
    def initialise_free_surfaces(self):
        ...
        self.stokes_vars[2].interpolate(
            self.F0 * cos(self.number_of_lam * atan2(self.X[1], self.X[0]))
    ...
    def setup_nullspaces(self):
        ...
        self.Z_nullspace = create_stokes_nullspace(
            self.Z, closed=False, rotational=True
        )
        self.Z_near_nullspace = create_stokes_nullspace(
            self.Z, closed=False, rotational=True, translations=[0, 1]
        )
    ...
    def update_analytical_free_surfaces(self):
        ...
        self.eta_analytical.interpolate(
            exp(-self.time / self.tau0)
            * self.F0
            * cos(self.number_of_lam * atan2(self.X[1], self.X[0]))
        )
