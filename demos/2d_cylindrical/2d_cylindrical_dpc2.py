...
Q = FunctionSpace(mesh, "DPC", 2)
limiter = VertexBasedDPC2Limiter(Q)
...
for timestep in range(0, max_timesteps):
    ...
    if timestep % dump_period == 0:
        output_file.write(u, p, T, limiter.alpha1, limiter.alpha2, limiter.q[0], limiter.q[1])
    ...
    plog.log_str(f"{timestep} {time} {float(delta_t)} {maxchange} {u_rms} "
                 ...
                 f"{energy_conservation} {average_temperature} {Tmin} {Tmax} "
                 f"{stokes_solver.steady_state_residual_norm()} "
                 f"{energy_solver.steady_state_residual_norm()}")
