...
def setup_mesh_and_spaces():
    ...
    """
    Initialises the mesh hierarchy and defines the DG function spaces.
    """
    # Configuration
    refinement_levels = 3
    number_layers = 300
    horiz_res_fine = 1775
    horiz_res_coarse = horiz_res_fine * (2**refinement_levels)

    # --- Build Mesh Hierarchy ---
    surface_mesh(horiz_res_coarse)
    if refinement_levels > 0:
        m2d = Mesh("Murrumbidgee_SurfaceMesh.msh")
        mh2d = MeshHierarchy(m2d, refinement_levels=refinement_levels)
        layer_list = [number_layers * (1) for i in range(refinement_levels + 1)]
        mh3d = ExtrudedMeshHierarchy(mh2d, height=1.0, layers=layer_list)

        # Apply terrain mapping
        mh3d = apply_terrain_transformation(mh3d, 'bedrock_data.csv', 'elevation_data.csv')

        # Re-wrap hierarchy (standard Firedrake procedure for moved meshes)
        # mh3d_final = HierarchyBase(
        #   list(mh3d), 
    ...
    else:  # Don't bother making mesh hierarchy
    ...
    # Using Tensor Product Elements for DG Discretisation
    ...
    # --- Load Spatial Data ---
    W_geo = VectorFunctionSpace(mesh, 'CG', 1)
    mesh_coords = assemble(interpolate(mesh.coordinates, W_geo)).dat.data_ro

    spatial_data = {
        'elevation':    load_spatial_field("Elevation", "elevation_data.csv", V, V_cg, mesh_coords),
        'shallowLayer': load_spatial_field("Shallow", "shallow_layer.csv", V, V_cg, mesh_coords),
        'lowerLayer':   load_spatial_field("Lower", "lower_layer.csv", V, V_cg, mesh_coords),
        'bedrock':      load_spatial_field("Bedrock", "bedrock_data.csv", V, V_cg, mesh_coords),
        'watertable':   load_spatial_field("WaterTable", "water_table.csv", V, V_cg, mesh_coords),
        'rainfall':     load_spatial_field("Rainfall", "rainfall_data.csv", V, V_cg, mesh_coords),
    }
    
    # Calculate depth relative to surface
    x = SpatialCoordinate(mesh)
    elev = spatial_data['elevation']
    spatial_data['depth'] = Function(V, name='depth').interpolate(
        conditional(elev - x[2] < 0, 0.0, elev - x[2])
    )

    PETSc.Sys.Print("Refinement levels:", refinement_levels)
    PETSc.Sys.Print("The number of degrees of freedom is:", V.dim())
    PETSc.Sys.Print("Horizontal resolution:", horiz_res_fine)
    PETSc.Sys.Print("Number of layers:", number_layers)
    PETSc.Sys.Print(horiz_elt, vert_elt)

    return mesh, V, W, spatial_data

def apply_terrain_transformation(mh3d, bedrock_file, elevation_file):

    """
    Transforms an extruded mesh hierarchy to follow terrain and bedrock surfaces.
    """

    """
    # Apply vertical stretching/scaling (currently seems to destroy boundary IDs)
    ...
    for mesh in mh3d:
        ...
        # Power law stretching (s**1.0 is linear)
        new_coords_expr = as_vector([x, y, s**1.0])
        new_coords_fn = project(new_coords_expr, coord_space)
        mesh.coordinates.assign(new_coords_fn)
    """

    # Map to physical bedrock and elevation data (CG1) spaces for the geometry transformation
    V_geo_coarse = FunctionSpace(mh3d[0], "CG", 1)
    coords_coarse = mh3d[0].coordinates

    bed0 = Function(V_geo_coarse)
    elv0 = Function(V_geo_coarse)
    bed0.dat.data[:] = data_2_function(coords_coarse.dat.data_ro, bedrock_file)
    elv0.dat.data[:] = data_2_function(coords_coarse.dat.data_ro, elevation_file)

    beds, elvs = [bed0], [elv0]
    for lev in range(1, len(mh3d)):
        Vi = FunctionSpace(mh3d[lev], "CG", 1)
        bed, elv = Function(Vi), Function(Vi)
        prolong(beds[-1], bed)
        prolong(elvs[-1], elv)
        beds.append(bed)
        elvs.append(elv)
    ...
    for lev, m in enumerate(mh3d):
        ...
        z = coords.dat.data[:, 2]
        b, e = beds[lev].dat.data[:], elvs[lev].dat.data[:]
        # Linear interpolation between bedrock and surface
        coords.dat.data[:, 2] = b * z + e - b
...
def define_soil_curve(mesh, V, spatial_data):
    ...
    depth        = max_value(spatial_data['depth'], 0.0) # Probably not nessessary

    # Indicator functions I1, I2 transition between shallow, lower, and bedrock layers.
    delta = 0.2 # Transition thickness
    ...
    Ks_layer1 = 2.5e-05  # Shapperton formation
    Ks_layer2 = 1e-03    # Calivil Formation
    Ks_layer3 = 5e-04    # Renmark group
...
def setup_boundary_conditions(mesh, h, spatial_data):
    ...
    rain_scaling = 0.14 * 3.171e-11  # Percentage of rainfall that enters ground & converts mm/y to m/s

    richards_bcs = {
        'bottom' : {'flux': 0},
        'top'    : {'flux': rain_scaling*rainfall},
        3        : {'flux': -(h - (depth - watertable))},               # Side boudaries
    }
...
def main():
    ...
    soil_curve = define_soil_curve(mesh, V, spatial_data)
    ...
    h_old = Function(V, name="PressureHeadOld").assign(initial_head)

    richards_bcs = setup_boundary_conditions(mesh, h, spatial_data)
    ...
    K = Function(V, name='Relative_conductivity').interpolate(relative_conductivity(h))
    q = Function(W, name='Volumetric_Flux').interpolate(-K*grad(h + x[2]))
    ...
    while current_time < t_final:
        ...
        water_mass = assemble(theta*dx(degree=3))
