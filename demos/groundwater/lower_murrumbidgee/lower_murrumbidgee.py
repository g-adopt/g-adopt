...
def setup_mesh_and_spaces():
    ...
    refinement_levels = 0
    number_layers = 300
    horiz_res = 1500 * (2**refinement_levels)

    # --- Build Mesh Hierarchy ---
    surface_mesh(horiz_res)
    m2d = Mesh("Murrumbidgee_SurfaceMesh.msh")
    mh2d = MeshHierarchy(m2d, refinement_levels=refinement_levels)
    mh3d = ExtrudedMeshHierarchy(mh2d, height=1.0, base_layer=number_layers)
    ...
    mh3d_final = HierarchyBase(
        list(mh3d), mh3d.coarse_to_fine_cells, mh3d.fine_to_coarse_cells,
        refinements_per_level=mh3d.refinements_per_level, nested=mh3d.nested
    )
...
def apply_terrain_transformation(mh3d, bedrock_file, elevation_file):
    ...
    """
    # Apply vertical stretching/scaling
    for mesh in mh3d:
...
def main():
    ...
    #q = Function(W, name='Volumetric_Flux').interpolate(-K*grad(h + x[eq.dim-1]))
