...
def forward_problem():
    ...
    with CheckpointFile("./Adjoint_CheckpointFile.h5", "r") as f:
        ...
        mu_radial = f.load_function(mesh, name="mu1viscosity")
    ...
    Di = Constant(0.5)  # Dissipation number.
    mu_function = Function(mu_radial.function_space(), name="Viscosity")
    delta_mu_T = Constant(100.)
    mu_T = mu_radial * exp(-ln(delta_mu_T) * (T - Tave))
    mu = conditional(mu_T > 0.4, mu_T, 0.4)
