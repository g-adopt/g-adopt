...
mpiexec --map-by ppr:1:node -np $PBS_NNODES tar -C $PBS_JOBFS -xf /g/data/xd2/sg8812/image/firedrake-prefix-upstream-full-openmpi${openmpi_version//./}-${petsc_arch}.tar.gz
...
mpiexec -np 1344 python3 gplates_NL_compressible.py &> output.dat
# mpiexec -np 1344 python3 load_reload.py &> load_reload.dat
