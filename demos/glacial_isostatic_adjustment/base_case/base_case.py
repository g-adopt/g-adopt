...
#
# The density field is decomposed into a time-independent background component, $\rho_0$, and a small time-dependent perturbation, $\rho_1$, such that $\rho = \rho_0 + \rho_1$. Substituting this decomposition yields the linearised mass conservation equation
#
# \begin{equation}
#     \partial_t \rho_1 + \nabla \cdot (\rho_0\, \boldsymbol{v}) = 0,
# \end{equation}
#
# where we have neglected the $\nabla \cdot (\rho_1\, \boldsymbol{v})$ term, by assuming that perturbations in density are small compared with background values. Integrating this equation in time and using the relation $\partial_t \boldsymbol{u} = \boldsymbol{v}$, where $\boldsymbol{u}$ is the non-dimensional displacement vector, we obtain the following expression for the density perturbation
#
# \begin{equation}
#     \rho_1(t) = - \nabla \cdot \left( \rho_0\, \boldsymbol{u}(t) \right) = - \rho_0 \nabla \cdot \boldsymbol{u}(t) - \boldsymbol{u}(t) \cdot  \nabla \rho_0 ,
# \end{equation}
#
# assuming a vanishing initial displacement, $\boldsymbol{u}(t=0) = \textbf{0}$. The density perturbation $\rho_1$ is often referred to as the *Eulerian density pertubation* by the GIA community.
#
# The full GIA equations include rotational and gravitational effects arising from changes in the surface load as water is redistributed between ice and ocean, which is governed by the sea-level equation. Neglecting those terms for now, alongside inertial terms, yields the following non-dimensional form of the momentum equation
#
# \begin{equation}
...
# -------------
#
# To further simplify the momentum equation, we linearise perturbations in the stress field by expressing the total stress as a sum of a time-invariant hydrostatic background component and a perturbation: $\boldsymbol{\sigma} = \boldsymbol{\sigma}_0 + \boldsymbol{\sigma}_1^E$, where the superscript $E$ indicates an Eulerian perturbation. Since viscoelastic constitutive relations that link stress to strain (and their time derivatives) are fundamentally defined in a Lagrangian framework, we also need to transition from an Eulerian to a Lagrangian description of the stress field. To capture the material response in a Lagrangian framework, the perturbation must account for the change in stress due to displacement of material through the background stress gradient. To first order, the incremental Lagrangian stress is $\boldsymbol{\sigma}_1^L = \boldsymbol{\sigma}_1^E + \boldsymbol{u} \cdot \nabla \boldsymbol{\sigma}_0$ see Eq. 3.16 of Dahlen and Tromp (1998). Substituting, we obtain
#
# \begin{equation}
...
# \end{equation}
#
# Moreover, assuming that the background density $\rho_0$ varies only in the radial direction, the advection of hydrostatic pre-stress simplifies to
#
# \begin{equation}
# \boldsymbol{u} \cdot \nabla \boldsymbol{\sigma}_0 = B_{\mu}\, \rho_0\, g\,u_k\, \boldsymbol{I},
# \end{equation}
#
# where $u_k$ is the component of non-dimensional displacement in the $\boldsymbol{\hat{e}}_k$ direction and $\boldsymbol{I}$ is the identity tensor. Substituting these expressions  leads to the final linearised form of the momentum equation
#
# \begin{equation}
...
# term becomes leading order when the wavelength is greater than 30,000 km for typical Earth parameters, i.e. only
...
# ----------------
#
# The GIA community generally model the mantle as a Maxwell solid. The conceptual picture is a
...
#
#
# where $\kappa$ is the non-dimensional bulk modulus and $\mu_0$ is the non-dimensional effective shear modulus given by
# \begin{equation}
#     \mu_0 = \sum_i \mu_i
# \end{equation}
...
DG0 = FunctionSpace(mesh, "DQ", 0)  # (Discontinuous) Density/viscosity/shear modulus function space (scalar)
...
# We now specify a function to store our displacement solution. We also need to initialise a function to store the old value of the internal variable. Since we are assuming Maxwell rheology
# we only need one internal variable.
#

# +
u = Function(V, name="displacement")  # field to hold our displacement solution

m = Function(S, name="Internal variable")  # Lagged value of internal variable at previous timestep
m_list = [m]
# -
...
# our lithosphere, has a Maxwell time of 6 x10$^{21}$ years. Given that our simulations only run for 110,000 years the viscous
...
# choose a timestep of 1000 years and an output frequency of 2000 years.
...
approximation = CompressibleInternalVariableApproximation(bulk_modulus=bulk_modulus, density=density, shear_modulus=[shear_modulus], viscosity=[viscosity], Vi=Vi)

# We finally come to solving the variational problem, with solver
# objects for the Stokes system created. We pass in the solution fields `u` and various fields
...
# by a factor of 500, **BUT...** it is important to remember this is just for ease of visualisation -
