...
# Set up temperature function space for DG:
cell = mesh.ufl_cell()
elt = FiniteElement("DG", cell, 1, variant='equispaced')
Q = FunctionSpace(mesh, elt)
limiter = VertexBasedQ1DGLimiter(Q)
...
for timestep in range(0, max_timesteps):
    ...
    if timestep % dump_period == 0:
        output_file.write(u, p, T, limiter.alpha1, limiter.alpha2)
    ...
    # Temperature checks:
    Tmin = mesh.comm.allreduce(T.dat.data.min(), MPI.MIN)
    Tmax = mesh.comm.allreduce(T.dat.data.max(), MPI.MAX)
    Tvmin = mesh.comm.allreduce(limiter.min_field.dat.data.min(), MPI.MIN)
    Tvmax = mesh.comm.allreduce(limiter.max_field.dat.data.max(), MPI.MAX)
    Tcmin = mesh.comm.allreduce(limiter.centroids.dat.data.min(), MPI.MIN)
    Tcmax = mesh.comm.allreduce(limiter.centroids.dat.data.max(), MPI.MAX)
    ...
    plog.log_str(f"{timestep} {time} {float(delta_t)} {maxchange} {gd.u_rms()} {gd.u_rms_top()} {ux_max} "
                 ...
                 f"{energy_conservation} {gd.T_avg()} {Tmin} {Tmax} {Tvmin} {Tvmax} {Tcmin} {Tcmax} ")
