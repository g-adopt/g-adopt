...
class ExplicitFreeSurfaceModel:
    ...
    def __init__(self, dt_factor, nx=80, do_write=False):
    ...
    def setup_solver(self):
        ...
        self.stokes_solver = StokesSolver(self.z, self.T, self.approximation, bcs=self.stokes_bcs, mu=self.mu, cartesian=True, free_surface_dt=self.dt)

        # Setup remaining free surface parameters needed for explicit coupling
        eta_eq = FreeSurfaceEquation(self.W, self.W, free_surface_id=self.top_id, k=upward_normal(self.mesh, cartesian=True))  # Initialise the separate free surface equation for explicit coupling
    ...
    def calculate_error(self):
        local_error = assemble(pow(self.eta-self.eta_analytical, 2)*ds(self.top_id))
