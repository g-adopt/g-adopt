...
ds_tb = ds_t + ds_b
# Set up geometry:
rmin, rmax, nlayers = 1.22, 2.22, 64
...
domain_volume = assemble(1.0 * dx(domain=mesh))
...
time = (409 - 5.0) * (
    libgplates.myrs2sec * libgplates.plate_scaling_factor / libgplates.time_dim_factor
)
time_presentday = 409 * (
    libgplates.myrs2sec * libgplates.plate_scaling_factor / libgplates.time_dim_factor
)
...
(
    u_,
    p_,
) = z.subfunctions  # Do this first to extract individual velocity and pressure fields.
...
Told.project(Tic, solver_parameters=project_solver_parameters, bcs=[bctb, bctt])
Tnew.assign(Told)
...
t_misfit = assemble((Tnew - Tobs) ** 2 * dx)

# objective = t_misfit + 0.01 * (norm_obs * smoothing / norm_smoothing)
objective = t_misfit
