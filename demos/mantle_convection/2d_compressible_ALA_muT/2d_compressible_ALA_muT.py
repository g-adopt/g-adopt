...
# ================================================================
#
# Our previous tutorial, examined convection under the Anelastic Liquid
# Approximation (ALA), can be easily modified to account for a temperature dependent viscosity,
...
# under ALA. We specify $Ra=10^4$ and a dissipation number $Di=0.5$.
...
#
# Key differences relative to our previous tutorial are:
# 1. We incorporate a temperature dependent viscosity.
#
...
# Given that viscosity depends on temperature here, we setup and initialise our temperature
# earlier than in the previous tutorials. Note that we take into consideration
...
# We subsequently specify how our viscosity depends on temperature and create a field for visualisation:

gamma_T = Constant(ln(10**3))
mu = exp(-gamma_T*FullT)
mu_field = Function(W, name="Viscosity")  # For vis only

# These fields are used to set up our Anelastic Liquid Approximation, which includes our
# expression for the viscosity field.
...
for timestep in range(0, timesteps):
    ...
    if timestep % output_frequency == 0:
        ...
        output_file.write(*z.subfunctions, T, FullT, mu_field)
