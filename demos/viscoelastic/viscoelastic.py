...
# $\Phi$ is the gravitational potential field. As with mantle convection, we have
...
# ------------------------------------
# The GIA community usually reformulates the linearised momentum equation in terms of
# the *Incremental lagrangian stress tensor*. This can be traced back to the early roots
# of GIA modellers adopting Laplace transform methods. The idea behind this is to
# convert the time dependent *viscoelastic* problem to a time independent *elastic*
# problem by the correspondence principle.

# From an elastic wave theory point of view, Dahlen and Tromp (1998) make the point that
# it is the Lagrangian perturbation in stress not the Eulerian perturbation that is
# related to the displacement gradient by the elastic parameters. Transforming between
# the Lagrangian perturbation in stress and the Eulerian description is given by
...
# Cathles (1975) estimates that the term becomes leading order when the wavelength is
# greater than 30,000 km for typical Earth parameters, i.e. only when the wavelength is
# the same order of magnitude as the circumference of the Earth.
...
# include this term a load placed on the surface of the Earth will keep sinking (Wu and
# Peltier, 1982)!
...
# conceptual picture is a spring and a dashpot connected together in series (Ranalli,
# 1995). For this viscoelastic model the elastic and viscous stresses are the same but
# the total displacements combine.
...
# where $\overset{\cdot}{\boldsymbol{\epsilon}}^v $ is the viscous strain rate tensor,
# $\boldsymbol{\epsilon}^e$ is the elastic strain tensor, $\eta$  is the viscosity,
# $\mu$ is the shear modulus, $\textbf{I}$ is the Identity matrix, $p$ is the pressure,
# and $\boldsymbol{\sigma}_{L1} $ is the incremental Lagrangian stress tensor. Note $p$
# is a perturbation pressure as we have already removed the hydostatic background state
# earlier. An overhead dot notes the time derivative i.e the viscous strain rate is
# proportional to stress, while the elastic strain  is proportional to stress. The total
# strain is
...
# we will leave solving the associated Poisson equation to a later demo.

# Time discretisation
# -------------------

# One of the key differences with the mantle convection demos is that the constitutive
# equation now depends on time. G-ADOPT implements the method of Zhong et al. (2003)
# where deviatoric stress is accounted for via an 'incremental displacement', thus
# recasting the problem in terms of
# $\textbf{u}_{inc}^n = \textbf{u}^n - \textbf{u}^{n-1}$, where subscripts refer to time
# levels $t$ and $t - \Delta t$ respectively. The incremental strain
# $\Delta \boldsymbol{\epsilon}$ is
...
# This example
# ------------
# We will simulate a viscoelastic loading and unloading problem based on a 2D version of
# the test case presented in Weerdesteijn et al. (2023).
...
# Weerdesteijn test case is a 3D box 1500 km wide horizontally and 2891 km deep. To
# speed up things for this first demo, we consider a 2D domain, i.e. taking a vertical
# cross section through the 3D box.

# For starters let's use one of the default meshes provided by Firedrake,
# `RectangleMesh`. We have chosen 40 quadrilateral elements in the $x$ direction and 40
# quadrilateral elements in the $y$ direction. It is worth emphasising that the setup
# has coarse grid resolution so that the demo is quick to run! For real simulations we
# can use fully unstructured meshes to accurately resolve important features in the
# model, for instance near coastlines or sharp discontinuities in mantle properties. We
# can print out the grid resolution using `log`, a utility provided by G-ADOPT.
# N.B. `log` is equivalent to Python's `print` function but only allows the root process
# to write to stdout when running simulations in parallel.
...
# symbolic variables representing function spaces. They also contain the function
# space's computational implementation, recording the association of degrees of freedom
# with the mesh and pointing to the finite element basis. We will choose Q2-Q1 for the
# mixed incremental displacement-pressure similar to our mantle convection demos. This
# is a Taylor-Hood element pair which has good properties for Stokes modelling. We also
# initialise a discontinuous tensor function space that will store our previous values
# of the deviatoric stress, as the gradient of the continous incremental displacement
...
initialise_background_field(mu, mu_values)
# -
...
# lithosphere, has a Maxwell time of 6 million years. Given that our simulations only
# run for 110,000 years the viscous deformation over the course of the simulation will
# always be negligible compared with the elastic deformation. For now let's choose a
# timestep of 250 years and an output frequency of 2000 years.
...
# `ramp` parameter. At each step we call `solve` to calculate the incremental
# displacement and pressure fields. This will update the displacement at the surface and
# stress values accounting for the time dependent Maxwell consitutive equation.
...
#     arrows = data.glyph(
#         orient="Incremental displacement",
#         scale="Incremental displacement",
#         factor=4e5,
#         tolerance=0.05,
#     )
...
#         (750000.0, 1445500.0, 6291991.008627122),
...
# Looking at the animation, we can see that as the weight of the ice load builds up the
# mantle deforms, pushing up material away from the ice load. If we kept the ice load
# fixed this forebulge will eventually grow enough that it balances the weight of the
# ice, i.e the mantle is in isostatic equilbrium and the deformation due to the ice load
# stops. At 100,000 years when the ice is removed the topographic highs associated with
# forebulges are now out of equilibrium so the flow of material in the mantle reverses
# back towards the previously glaciated region.
...
# Heister, T., & Zhang, J. (2023)
# *Modeling viscoelastic solid earth deformation due to ice age and contemporary glacial
# mass changes in ASPECT*
