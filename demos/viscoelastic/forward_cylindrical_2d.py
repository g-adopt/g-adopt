...
class ForwardCylindrical2d(SpadaCylindrical2d):
    ...
    def setup_heterogenous_viscosity(self):
        ...
        self.heterogenous_viscosity_field.interpolate(-3*low_viscosity_antarctica + self.viscosity * (1-low_viscosity_antarctica))
        
        llsvp1_x, llsvp1_y = 3.5e6, 0
        llsvp1 = bivariate_gaussian(self.X[0], self.X[1], llsvp1_x, llsvp1_y, 0.75e6, 1e6, 0)
        self.heterogenous_viscosity_field.interpolate(-3*llsvp1 + self.heterogenous_viscosity_field * (1-llsvp1))
        
        llsvp2_x, llsvp2_y = -3.5e6, 0
        llsvp2 = bivariate_gaussian(self.X[0], self.X[1], llsvp2_x, llsvp2_y, 0.75e6, 1e6, 0)
        self.heterogenous_viscosity_field.interpolate(-3*llsvp2 + self.heterogenous_viscosity_field * (1-llsvp2))

        slab_x, slab_y = 3e6, 4.5e6
        slab = bivariate_gaussian(self.X[0], self.X[1], slab_x, slab_y, 0.7e6, 0.35e6, 0.7)
        self.heterogenous_viscosity_field.interpolate(-1*slab + self.heterogenous_viscosity_field * (1-slab))
        ...
        self.heterogenous_viscosity_field.interpolate(-1*high_viscosity_craton + self.heterogenous_viscosity_field * (1-high_viscosity_craton))
