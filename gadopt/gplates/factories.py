...
class PolygonConnectorFactory(ConnectorFactory):
    ...
    def construct_output(
        ...
        *,
        ...
        ``default_thickness_km`` defaults to 0 here: PolygonSource fills the
        exterior with a fresh zero-thickness background around the in-polygon
        seeds, so a ``too_far`` target node should read as "outside the
        region" rather than filling in 100 km of fake material.
        """
        super().construct_output(
            fade_ref_km=fade_ref_km,
            width_km=width_km,
