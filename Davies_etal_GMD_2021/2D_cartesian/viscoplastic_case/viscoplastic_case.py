...
for timestep in range(0, max_timesteps):
    ...
    max_viscosity = mu_f.dat.data.max()
    max_viscosity = mu_f.comm.allreduce(max_viscosity, MPI.MAX)
    min_viscosity = mu_f.dat.data.min()
    min_viscosity = mu_f.comm.allreduce(min_viscosity, MPI.MIN)
    rate_work_against_gravity = assemble(Tnew * u[1] * dx) / domain_volume
    rate_viscous_dissipation = (assemble(inner(stress, epsilon) * dx) / domain_volume) / Ra.values()[0]
