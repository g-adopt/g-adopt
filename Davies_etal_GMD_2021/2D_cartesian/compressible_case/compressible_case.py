...
Told.interpolate((1.0-T0) * ((1.0-X[1]) + (0.05*cos(pi*X[0])*sin(pi*X[1]))))
Tnew.assign(Told)
...
for timestep in range(0, max_timesteps):
    ...
    if timestep % dump_period == 0:
        output_file.write(u, p, Tnew, FullT)
    ...
    nusselt_number_top = -1 * assemble(dot(grad(FullT), n) * ds(top_id))
    nusselt_number_base = assemble(dot(grad(FullT), n) * ds(bottom_id))
    ...
    rate_viscous_dissipation = (assemble(inner(stress, grad(u)) * dx) / domain_volume) * Di.values()[0]/Ra.values()[0]
